...
La documentation complète et interactive de ce projet est propulsée par **Zensical**. Pour la générer et la consulter en local avec son thème et sa navigation personnalisée, suiver ces étapes :
...
Ouvrir ensuite votre navigateur web et se rendre à l'adresse **[http://localhost:8000](http://localhost:8000)** pour parcourir les ateliers interactifs.
...
- **[`challenges-serveur-web/`](docs/challenges-serveur-web/)** : Création d'un playbook capable de s'adapter automatiquement à la distribution cible (Debian, Rocky, SUSE) pour installer Apache en s'appuyant sur les *Ansible facts* (`ansible_os_family`).
